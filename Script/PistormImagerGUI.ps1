...
function Write-AmigaInfoType {
    ...
    & $HSTAmigaPathtouse icon update $IconPath -t  $HSTParameter >$Logoutput
    $CheckforError = Get-Content ($Logoutput)
    $ErrorCount=0
    foreach ($ErrorLine in $CheckforError){
        if ($ErrorLine -match " ERR]"){
...
function Get-ListofInstallFiles {
    param (
        ...
        # $ListofInstallFilesCSV = "C:\Users\Matt\OneDrive\Documents\Emu68Imager\InputFiles\ListofInstallFiles.csv"
